...
int main() {
    DIR *dir = opendir("./"); // open dir
    if (dir == nullptr)
        throw std::runtime_error(OPEN_DIR_PREFIX + last_error());  // check, if result null, throw an error

    errno = 0; // clear error
    struct dirent *dir_entry = readdir(dir); // create dirent
    do {
        dir_entry = readdir(dir); // read dir/dir_entry
        string name(dir_entry->d_name); // define name as string

        if (dir_entry == nullptr) continue; // check is dir/dir_entry null

        if (errno != 0)
            throw std::runtime_error(RED_DIR_PREFIX + last_error()); // If any error acquired
                                                                     // while reading, check and throw error
        ...
        if (name != "." && name != ".." &&
            ...
            if (rename(dir_entry->d_name, new_name.c_str())) // rename
